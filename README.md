...
이 저장소는 현재 **Flutter + Firebase** 기반으로 구성되어 있으며, 로그인과 회원가입, 이메일 인증, 사용자 프로필 저장, 홈 피드, 내 그룹 분리, 그룹 생성, 그룹 참여까지 연결되어 있습니다.
...
- 이후 기능 확장을 통해 필터링, 그룹 상세, 채팅, 알림까지 이어지는 구조를 목표로 합니다.
...
메인 화면에서는 `group` 컬렉션의 문서를 읽어 Home 탭의 참여 가능 그룹과 My Groups 탭의 호스팅/참여 그룹을 보여줍니다.
